...
Pipeline pricing follows a transparent, usage-based model that reflects the true costs of running real-time AI inference on video streams. This guide explains how pricing works and what to expect when using pipelines in your applications.

## The Basics

You pay only for the time your streams use a pipeline. Pricing is calculated per minute of video processed, with costs starting when a stream connects and ending when it disconnects.

```
Total Cost = Minutes Streamed × Pipeline Rate
```
...
1. **Base Compute Cost**: The actual infrastructure cost to run the pipeline
...
You can also choose a specific provider if you prefer. This might be useful if you:
...
```
1 hour stream = 60 minutes × $0.10 = $6.00
24 hour stream = 1,440 minutes × $0.10 = $144.00
```
